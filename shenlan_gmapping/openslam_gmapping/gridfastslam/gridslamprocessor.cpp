...
# endif
  ...
  可以看m_matcher.setMatchingParameters函数里面
  urange
  range
  sigma
  kersize
...
//  }
  ...
  {
      /*
      FIXME For now detect the readings of only the front laser, and 
      assume its pose is in the center of the robot 
      */
      // 之前在initMapper里已经插入了"FLASER"和相应gsp_laser_
      SensorMap::const_iterator laser_it=smap.find(std::string("FLASER"));
      // 如果没找到，就找carmen log格式的数据
      ...
      // 以下其实其实是RangeSensor构造函数里的变量又取出来
